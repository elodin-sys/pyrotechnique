...
| Sprite textures | `write_soft_circle` / `write_smoke_puff` in builders | Regenerated by `gen-effects`. Earth / skybox files are **not** regenerated — do not overwrite them. |
...
`h = 400_000`. `SkyRoot` (sun + star emitters + cubemap rotation) rotates
about +X once per 90 s sim orbit: t=0 noon, 22.5 dusk, 45 midnight, 67.5 dawn.
Earth is `earth_v5.glb`. Skybox brightness is 0 at noon.
